...
import {
  buildCargoFleetCascade,
  enrichRoutesCargo,
} from "../cargo/cargoRoutes";
...
export function runGlobalOptCargo(cargoAircrafts, routes) {
  // ══════════════════════════════════════════════════════════════════════════
  // CARGO — Structure alignée sur l’optimiseur PAX : 168h → 84h×2 → 24h → rescue
  ...
  while (prog84 && p84++ < 20) {
    ...
    for (const ac of sortedAc) {
      ...
      while (p++ < 100) {
        ...
        if (!chosen || chosen.length < 2) break;
        chosen.forEach((r) => lu.add(r.id));
        const c = makeCircuit(ac, chosen, 84, 2);
        if (c.totalProfit <= 0) continue;
        all84.push(c);
        chosen.forEach((r) => used.add(r.id));
        prog84 = true;
